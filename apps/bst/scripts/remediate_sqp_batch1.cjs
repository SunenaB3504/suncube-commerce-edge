...
function processUnit(unitNum) {
    ...
    while ((match = questionRegex.exec(content)) !== null) {
        ...
        // Let's create an expanded synthetic answer and AI explanation for any 3-6 mark question 
        // based on the marks and standard NCERT expectations.
        let expandedAnswer = "";
        let aiExplanation = "";
        ...
        // We do a regex replace inside the block for answer, markingScheme, aiExplanation
        block = block.replace(/answer:\s*[`'"][\s\S]*?[`'"]/, `answer: \`${expandedAnswer}\``);
        block = block.replace(/markingScheme:\s*[`'"][\s\S]*?[`'"]/, `markingScheme: \`${expandedAnswer}\``);
    ...
    if (modifications > 0) {
        ...
        console.log(`Unit ${unitNum}: Artificially expanded ${modifications} high-mark subjective questions.`);
    }
}
